...
- [ ] **Task 1: Inventory Sync Data Model** (AC: 1, 9)
  - [ ] Create `apps/lab-lite/src/lib/inventory/network-sync.ts`.
  - [ ] Define outbound inventory snapshot payload:
    ```typescript
    ...
    ```
  - [ ] Verify payload contains zero PHI — reagent quantities and facility data only.

- [ ] **Task 2: Inventory Sync Worker** (AC: 1, 10)
  - [ ] Create `apps/lab-lite/src/lib/inventory/sync-worker.ts`.
  - [ ] Build snapshot from local Dexie inventory tables (from Epic 48 reagent management).
  - [ ] Sync triggers:
  ...
  - [ ] Enqueue via `enqueueSyncAction()` with a custom resource type `InventorySnapshot` at sync priority 6 (same as Patient/demographics — operational, not clinical).
  - [ ] Store last successful sync timestamp in Dexie for offline indicator.

- [ ] **Task 3: Network Inventory Cache (Dexie)** (AC: 3, 10)
  ...
  - [ ] `NetworkInventoryEntry` schema:
    ...
    ```
  - [ ] Index: `++id, labId, district, snapshotAt`.
  - [ ] Retention: keep only the latest snapshot per lab (overwrite on update).

- [ ] **Task 4: Heat Map Dashboard Component** (AC: 3, 4, 5, 8)
  - [ ] Create `apps/lab-lite/src/components/inventory/NetworkInventoryHeatMap.tsx`.
  - [ ] Layout: matrix view with labs as rows, reagent categories as columns.
  - [ ] Cell color coding:
    - Green: `daysOfSupply > 30`
    - Yellow: `7 < daysOfSupply <= 30`
    - Red: `daysOfSupply <= 7` or `isStockedOut`
    - Gray: reagent not tracked at this lab
  - [ ] Stockout alert banner at top: list of labs with any critical stockout, with red background.
  - [ ] Click a cell to drill down: show detail for that lab + reagent (quantity, consumption rate, days of supply, expiry).
  - [ ] Toggle between "Network View" (all labs) and "My Lab" (single lab detail with network percentile).
  - [ ] RTL-ready: use logical CSS properties, ensure heat map reads correctly in both directions.
  - [ ] Responsive: stack to vertical list on narrow viewports.

- [ ] **Task 5: Redistribution Recommendation Engine** (AC: 6)
  - [ ] Create `apps/lab-lite/src/lib/inventory/redistribution-engine.ts`.
  - [ ] Algorithm:
  ...
  - [ ] Store recommendations in Dexie `redistributionRecommendations` table.
  - [ ] Recalculate on every network inventory update.

- [ ] **Task 6: Redistribution Recommendations UI** (AC: 6)
  - [ ] Create `apps/lab-lite/src/components/inventory/RedistributionPanel.tsx`.
  ...
  - [ ] Show only recommendations relevant to the current lab (where this lab is source or destination).
  - [ ] Network-admin role sees all recommendations across the district.

- [ ] **Task 7: Per-Lab vs Network View Toggle** (AC: 7, 8)
  - [ ] Create `apps/lab-lite/src/components/inventory/LabInventoryDetail.tsx`.
  - [ ] Per-lab view: detailed inventory table with all reagents, quantities, consumption rates, days of supply, expiry dates.
  - [ ] Network comparison: for each reagent, show this lab's `daysOfSupply` vs network average and percentile.
  - [ ] Visual indicator: bar chart showing this lab's position relative to network min/max/average.

- [ ] **Task 8: Offline Behavior** (AC: 10)
  - [ ] Network inventory view renders from Dexie cache when offline.
  - [ ] Display "Last synced: [timestamp]" banner when showing cached data.
  - [ ] Stale data warning: if last sync is >24 hours old, show yellow banner: "Network data may be outdated. Connect to update."
  - [ ] Redistribution recommendations still compute from cached data.

- [ ] **Task 9: Tests** (AC: 1-11)
  - [ ] Unit tests for inventory snapshot builder — verify zero PHI in payload.
  - [ ] Unit tests for redistribution engine — test stockout detection, peer matching, distance ranking, transfer amount calculation.
  - [ ] Unit tests for heat map color logic — boundary conditions (exactly 7 days, exactly 30 days, 0 quantity).
  - [ ] Integration test: simulate multi-lab inventory data, verify heat map renders correctly with all color states.
  - [ ] Offline test: verify cached data renders when Hub is unreachable.
  - [ ] RTL snapshot test for NetworkInventoryHeatMap and RedistributionPanel.
  - [ ] Test that redistribution recommendations do not suggest transfers that would deplete the source lab below 14 days.
