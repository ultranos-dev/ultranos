...
_Code review 2026-06-13 — 3 layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor). 24 findings: 6 decision-needed, 16 patch, 2 deferred, 0 dismissed._

#### Decision-Needed

- [x] [Review][Decision] D1: Wrong fields passed to `checkAndInitiateEscalation` — `orderingPhysicianId` receives `result.authorizedBy` (the authorizing lab tech, not the physician); `analyte` receives `result.testCategory` (a category label, not per-test analyte name). Both silently misdirect the escalation chain. Correct field names on `LabResultForAuthorization` required. [apps/lab-lite/src/lib/authorization-actions.ts:177-181]
- [x] [Review][Decision] D2: Fail-open on Dexie error in `isCriticalValue` — DB lookup failure returns `{ isCritical: false }` silently. Analogous to the "check unavailable" false-negative pattern prohibited by CLAUDE.md Rule #3 for drug interaction checks. Should show a "Critical value detection unavailable — verify manually" warning, or fail-closed and block result release until the DB recovers? [apps/lab-lite/src/lib/critical-value-engine.ts:89-92]
- [x] [Review][Decision] D3: Chain stuck at `currentStep: 1` after app restart — `catchUpMissedSteps` exempts step 1 (correct) but `advanceEscalation` also short-circuits for step 1, leaving the chain frozen with no timer progression and no re-shown full-screen alert. On restart, should the system: (A) re-emit the step 1 alert event for the UI to re-show, or (B) auto-advance past step 1 after a configurable grace period (e.g., if `createdAt` > N minutes ago)? [apps/lab-lite/src/lib/escalation-manager.ts:280, escalation-timer.ts]
- [x] [Review][Decision] D4: Step 2 (in-app notification to physician) delayed up to 60s after tech acknowledgment — timer resolution is 1-minute; step 2's `scheduledAt` is T+0 so it fires on the next tick (up to 60s later). Spec says step 2 fires "immediately after tech ack." Is up to 60s latency acceptable, or should `acknowledgeStep` for step 1 directly trigger step 2 without waiting for the next timer tick? [apps/lab-lite/src/lib/escalation-timer.ts, escalation-manager.ts:29-34]
- [x] [Review][Decision] D5: Raw `criticalValue` number displayed alongside `patientRef` in `EscalationStatusList` dashboard — together these constitute a PHI linkage (exact measurement + patient reference in same view). Spec says "no exact numeric value in SMS" but is silent on the dashboard. Should dashboard show direction (HIGH/LOW) + threshold only, or is the numeric value acceptable for clinical utility at the lab station? [apps/lab-lite/src/components/escalation/EscalationStatusList.tsx:122-127]
- [x] [Review][Decision] D6: Strict inequality in threshold comparison — value exactly equal to `criticalHigh` (e.g., Potassium = 6.5 mmol/L when `criticalHigh = 6.5`) is not flagged as critical (strict `>` not `>=`). Is this intentional clinical convention (configure threshold as "above X") or should boundary values be inclusive? [apps/lab-lite/src/lib/critical-value-engine.ts:63, 76]

#### Patch

- [x] [Review][Patch] P1: Missing escalation DB schema, types, and CRUD functions — `EscalationChain`, `EscalationStep`, `EscalationContact` types not in `db.ts`; `escalation_chains` + `escalation_contacts` Dexie tables never declared; 8 CRUD functions imported by `escalation-manager.ts` (`createEscalationChain`, `getEscalationChainById`, `updateEscalationChain`, `getActiveEscalationChains`, `getEscalationHistory`, `getDefaultEscalationContact`) and 4 by `ThresholdConfigPanel.tsx` (`getAllCriticalThresholds` → should be `getAllCriticalValueThresholds`, `putCriticalThreshold` → `putCriticalValueThreshold`, `deactivateCriticalThreshold`, `resetCriticalThresholdsToDefaults`) are all missing from `db.ts`. Feature is dead on arrival without this fix. [apps/lab-lite/src/lib/db.ts, apps/lab-lite/src/lib/escalation-manager.ts:17-24, apps/lab-lite/src/components/escalation/ThresholdConfigPanel.tsx:18-22, apps/lab-lite/src/components/escalation/EscalationContactsPanel.tsx:21]
- [x] [Review][Patch] P2: `escalation` i18n namespace missing from all 4 locale files — `useTranslations('escalation.alert')`, `('escalation.thresholds')`, `('escalation.status')`, `('escalation.contacts')` will throw at runtime in all 4 components. [apps/lab-lite/messages/en.json, ar.json, prs.json, ps.json]
- [x] [Review][Patch] P3: Audio `setInterval` handle leaked in `CriticalValueAlert.tsx` — `startAudio` returns a cleanup function but callers (`onClick={startAudio}`) discard the return value; the `useEffect` cleanup at line 133 does not clear the interval; after acknowledgment the closed AudioContext throws on every 2s beep tick. Store handle in `useRef`, clear in cleanup effect. [apps/lab-lite/src/components/escalation/CriticalValueAlert.tsx:104-131]
- [x] [Review][Patch] P4: Missed step events emitted before React listeners register on `resumeActiveEscalations` — `emitStep` fires synchronously at startup; all step event listeners (UI components) mount after this async function runs; missed-step notifications are silently dropped. Defer emission via `setTimeout(0)` or buffer events until first listener attaches. [apps/lab-lite/src/lib/escalation-timer.ts:111-113]
- [x] [Review][Patch] P5: Missing `/escalations` nav item in `AppSidebar.tsx` — `EscalationStatusList` page exists at `/[locale]/escalations` but no sidebar link points to it; active escalations are invisible to lab staff. [apps/lab-lite/src/components/AppSidebar.tsx]
- [x] [Review][Patch] P6: `critical-alert.mp3` missing from `/public/sounds/` — `CriticalValueAlert.tsx` references it as `<audio>` fallback; when Web Audio API is blocked by browser autoplay policy, no audio plays. [apps/lab-lite/public/sounds/critical-alert.mp3]
- [x] [Review][Patch] P7: `'sms_queue' in db` property check uses snake_case but Dexie table is `db.smsQueue` (camelCase) — condition is always `false`; no SMS is ever queued in Dexie via this path despite `{ queued: true }` being returned. [apps/lab-lite/src/lib/escalation-sms.ts:72]
- [x] [Review][Patch] P8: Medical director and district officer stored as `.name` (human-readable string) in `recipientId` field — PHI leak into step records and audit trail; SMS send path receives a name not a phone number. Use an opaque contact `id` as `recipientId`; resolve phone number from contacts table at send time. [apps/lab-lite/src/lib/escalation-manager.ts:123-124]
- [x] [Review][Patch] P9: Timer continues running up to 60s after chain acknowledged — `acknowledgeStep` never calls `stopEscalationTimer`; timer self-terminates only on next tick when it reads acknowledged status from Dexie. Call `stopEscalationTimer(chainId)` at end of `acknowledgeStep` for step > 1. [apps/lab-lite/src/lib/escalation-manager.ts:157-196, escalation-timer.ts:61-83]
- [x] [Review][Patch] P10: `ESCALATION_EXPIRED` audit event never emitted — `advanceEscalation` sets `status: 'expired'` in Dexie when step 5 completes but makes no `reportEscalationEvent` call; the audit type exists and is tested but never fires in the real code path. [apps/lab-lite/src/lib/escalation-manager.ts:238-244]
- [x] [Review][Patch] P11: `toggleActive` in `ThresholdConfigPanel` has no error handling — Dexie error propagates to async `onClick` handler with no try/catch and no user feedback; threshold activation state can silently fail to persist. [apps/lab-lite/src/components/escalation/ThresholdConfigPanel.tsx:103-115]
- [x] [Review][Patch] P12: `handleAcknowledge` in `EscalationStatusList` has no catch block — `acknowledgeStep` failure produces unhandled promise rejection; button re-enables via `finally` but user receives no indication the acknowledgment was not persisted. [apps/lab-lite/src/components/escalation/EscalationStatusList.tsx:93-110]
- [x] [Review][Patch] P13: Focus trap incomplete when sound prompt visible — `soundEnabled: false` state renders a 3rd interactive element (Enable Sound button) outside the managed `[checkboxRef, buttonRef]` array; Shift+Tab from checkbox escapes the modal to the sound button. Include sound button ref in `focusable` array conditionally. [apps/lab-lite/src/components/escalation/CriticalValueAlert.tsx:76-91, 217-223]
- [x] [Review][Patch] P14: `startAudio` attached to background `div` `onClick` — triggers audio on any click inside the modal (checkbox, acknowledge button) not only the "Enable Sound" button, bypassing the explicit opt-in UX. Remove `onClick={startAudio}` from the container `div`. [apps/lab-lite/src/components/escalation/CriticalValueAlert.tsx:193-194]
- [x] [Review][Patch] P15: `CRITICAL_VALUE_DETECTED` audit event uses placeholder `chainId: 'pre-initiation'` — not linkable to individual chain IDs in multi-critical results; auditor cannot join detection event to chain records. Emit one `CRITICAL_VALUE_DETECTED` event per critical finding inside the loop, after `initiateEscalation` returns the real `chainId`. [apps/lab-lite/src/lib/escalation-integration.ts:134-141]
- [x] [Review][Patch] P16: `handleAcknowledge` acknowledges `chain.currentStep` (next-to-fire pointer) not the last `'sent'` step — audit record logs the wrong step number when a physician acknowledges after an SMS was already sent; pass `steps.findLast(s => s.status === 'sent')?.stepNumber` instead. [apps/lab-lite/src/components/escalation/EscalationStatusList.tsx:97]

#### Deferred

- [x] [Review][Defer] W1: Test gap — no test covering L/H abnormality flags (vs LL/HH) in `deriveCriticalsFromFlags` path; a one-character rename could silently disable critical detection for most results. [apps/lab-lite/src/lib/escalation-integration.ts] — deferred, pre-existing test coverage pattern
- [x] [Review][Defer] W2: `resumeActiveEscalations` SSR guard (`typeof window === 'undefined'`) is untested — jsdom always defines `window` so the guard never fires in tests; no coverage that the function is a no-op in SSR contexts. [apps/lab-lite/src/__tests__/escalation-timer.test.ts:111] — deferred, pre-existing test pattern
