...
- [ ] [Review][Decision] **Routers don't wire encryption into read/write paths — AC1 & AC3 not met.** Medication, encounter, consent, and patient routers insert/update/select PHI columns (`medication_text`, `dosage_instruction`, `interaction_override`, `diagnosis`, `reason_code`, `soap_assessment`, `soap_plan`) without calling `db.toRow()`/`db.fromRow()` with encryption key. The crypto infrastructure is built but not integrated into any data path. User must decide: wire encryption into all routers now (expanding story scope) or create a follow-up story. [medication.ts, encounter.ts, consent.ts, patient.ts]
- [ ] [Review][Decision] **Encryption is opt-in via optional parameter — silent plaintext on omission.** `db.toRow(data, encryptionKey?)` makes no-encryption the default path. Any caller that forgets to pass the key silently writes plaintext PHI to the database with zero warning. User must decide: (A) make encryption mandatory with opt-out, (B) keep optional but add a runtime warning when PHI fields are written without encryption, or (C) accept current design. [supabase.ts:57-60]
