...
2. **ONE toolbar row** (`flex flex-wrap items-center gap-3`), always rendered (never behind a `loading`/`data.length>0` gate). **Element order is search-first:** **wide** `SearchInput` → filter **pill tabs** (selector bar) → filter `<select>` dropdown(s) → the **primary action folded into the END of this row** (Create/Export/Refresh/etc.). The search is the space-filler; do not push a lone action to the far right with `ms-auto` (that recreates the split-header anti-pattern). **Every control is `h-9` (36px) and the search + selects are `rounded-full`, matching the pill bar and the action buttons.**
   - **Search** — `SearchInput` from `@ultranos/ui-kit/components/ui/search-input`: `className="min-w-[200px] flex-1" inputClassName="h-9 rounded-full"` (the inner `Input` defaults to `h-10 rounded-xl`, so this override is required).
   - **Filter selector bar (pill tabs)** — the PRIMARY categorical dimension. Container `flex h-9 items-stretch gap-1 rounded-full border border-border bg-card p-1 w-fit`; buttons `flex items-center rounded-full px-4 text-sm font-medium transition-colors` (no `py-*` — the container height + `items-stretch` size them), active `bg-primary text-primary-foreground`, inactive `text-muted-foreground hover:text-foreground`.
   - **Filter dropdown (`<select>`)** — ONLY for a SECOND (or further) distinct dimension; **never duplicate the dimension the pill bar already covers**. Wrap in `relative` with a custom chevron (native `<select>` arrows are unstylable): `<div className="relative"><select className="h-9 w-full appearance-none rounded-full border border-border bg-background text-foreground ps-3 pe-9 text-sm">…</select><ChevronDown size={16} aria-hidden className="pointer-events-none absolute end-3 top-1/2 -translate-y-1/2 text-muted-foreground" /></div>` (`ChevronDown` from `@ultranos/ui-kit/icons`). Date-range inputs style each as `h-9 rounded-full border border-border bg-background px-3 text-sm` (two inputs = one dimension, allowed).
