...
export function EmptyState({ icon: Icon, title, description, action, testID }: EmptyStateProps) {
  ...
  return (
    <View testID={testID} style={styles.wrap}>
      ...
      <Text style={[styles.title, { color: colors.textPrimary }, rtl && styles.arabic]}>{title}</Text>
      {description ? <Text style={[styles.desc, { color: colors.textSecondary }, rtl && styles.arabic]}>{description}</Text> : null}
      {action ? (
