...
export function NotificationRow({ icon: Icon, appName, subject, body, notes, timeAgo, unread, urgent, unreadLabel, onClick }: NotificationRowProps) {
  return (
    <div role="button" tabIndex={0} onClick={onClick}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start gap-2">
          ...
          {unread && <span className="mt-1 inline-flex h-2 w-2 shrink-0 rounded-full bg-primary" aria-label={unreadLabel} aria-hidden={!unreadLabel} />}
          <span data-slot="notification-time" className="ms-auto shrink-0 text-xs text-muted-foreground">{timeAgo}</span>
        </div>
        <p className={`mt-0.5 text-sm ${urgent ? 'text-destructive' : 'text-foreground'}`}>{subject}</p>
