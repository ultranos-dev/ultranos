...
import { ZoomIn, ZoomOut, AlertCircle } from '../../icons.js'
...
const OUTPUT = 512                 // output canvas size px (server re-encodes to WebP)
const MAX_ZOOM = 5
...
interface PhotoCropperProps {
   ...
   * reads true to its circular display.
   */
  shape?: 'square' | 'circle'
  /** Accessible label for the zoom slider (defaults to "Zoom"). */
  zoomLabel?: string
  /** Error shown when the cropped image cannot be brought under 1 MB. */
  sizeErrorLabel?: string
}

// Clamp pan so the image always fills the crop frame
...
export const PhotoCropper = forwardRef<PhotoCropperHandle, PhotoCropperProps>(
  function PhotoCropper({ rawDataUrl, onCropped, shape = 'square', zoomLabel = 'Zoom', sizeErrorLabel = 'Image is too large after cropping. Try a smaller area.' }, ref) {
    ...
    const handleTouchStart = useCallback((e: React.TouchEvent<HTMLDivElement>) => {
      if (e.touches.length === 1) {
        ...
        const t0 = e.touches[0]; const t1 = e.touches[1]; if (!t0 || !t1) return
        setIsDragging(false)
        lastPinchDist.current = Math.hypot(t1.clientX - t0.clientX, t1.clientY - t0.clientY)
      }
    }, [transform.panX, transform.panY])

    const handleTouchEnd = useCallback(() => { setIsDragging(false); lastPinchDist.current = null }, [])
    ...
    const handleConfirm = useCallback(() => {
      ...
      if ((dataUrl.length - 23) * 0.75 > MAX_BYTES) { setError(sizeErrorLabel); return }
      onCropped(dataUrl)
    }, [onCropped, sizeErrorLabel])
    ...
    return (
      <div className="flex w-full flex-col items-center">
        ...
        >
          {rawDataUrl && (
            ...
            <img
              ...
              style={{
                ...
                maxWidth: 'none', maxHeight: 'none', userSelect: 'none', pointerEvents: 'none',
              }}
            />
          )}
          ...
          {shape === 'circle' ? (
            <div
              ...
              style={{ top: MARGIN, left: MARGIN, width: FRAME, height: FRAME, boxShadow: '0 0 0 9999px rgba(0,0,0,0.55)' }}
            />
          ) : (
            <>
        ...
        <div className="flex items-center gap-2 pt-3" style={{ width: VIEWPORT }}>
          <Button variant="ghost" size="icon-sm" type="button" aria-label="Zoom out" className="shrink-0" onClick={() => applyZoomFromCenter(transform.zoom * 0.9)}>
            <ZoomOut className="h-4 w-4" />
          </Button>
          <input
            type="range" min={0} max={100} value={zoomPct} aria-label={zoomLabel}
            onChange={(e) => { const pct = Number(e.target.value) / 100; applyZoomFromCenter(minZoom + pct * (MAX_ZOOM - minZoom)) }}
            className="flex-1 accent-primary"
          />
          <Button variant="ghost" size="icon-sm" type="button" aria-label="Zoom in" className="shrink-0" onClick={() => applyZoomFromCenter(transform.zoom * 1.1)}>
            <ZoomIn className="h-4 w-4" />
          </Button>
        </div>

        {error && (
          <div className="mt-2 flex items-center gap-2 rounded-xl border border-destructive/30 bg-destructive/10 px-3 py-2" style={{ width: VIEWPORT }}>
