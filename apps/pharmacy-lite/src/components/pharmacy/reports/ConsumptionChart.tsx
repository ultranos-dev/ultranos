  ...
  if (loading) {
    return (
      <div className="rounded-lg border border-neutral-200 bg-white p-4">
        <p className="text-sm text-neutral-500">Loading consumption data...</p>
      </div>
    )
  }
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-lg border border-neutral-200 bg-white p-4">
        ...
        <p className="text-sm text-neutral-500">No dispensing data available.</p>
      </div>
    )
  }
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.catalogItemId} className="flex items-center gap-3">
              <span className="w-32 shrink-0 truncate text-xs text-neutral-700">
                {item.catalogItemName}
              </span>
              <div className="flex-1">
                <div
                  className="h-5 rounded bg-green-500 transition-all"
                  style={{ width: `${widthPct}%`, minWidth: widthPct > 0 ? '4px' : '0' }}
                />
              </div>
              <span className="w-10 text-end text-xs tabular-nums text-neutral-600">
                {item.totalDispensed}
              </span>
            </div>
          )
