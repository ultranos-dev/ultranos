...
export interface UseNotificationPollResult {
  ...
  newNotifications: PharmacyNotification[]
  unreadCount: number
  loading: boolean
  error: string | null
...
/**
 * Shared polling hook for pharmacy notification system.
...
export function useNotificationPoll(intervalMs = POLL_INTERVAL_MS): UseNotificationPollResult {
  ...
  const [newNotifications, setNewNotifications] = useState<PharmacyNotification[]>([])
  const [unreadCount, setUnreadCount] = useState(0)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const activeRef = useRef(true)
  const notificationsRef = useRef<PharmacyNotification[]>([])
  /** Tracks IDs seen since first load. Seeded on first successful fetch. */
  const seenIdsRef = useRef<Set<string> | null>(null)

  const computeUnread = useCallback((items: PharmacyNotification[]) => {
