...
function makeCtx(user: { sub: string; role: string; sessionId: string; orgId?: string | null } | null) {
  ...
  const maybeSingle = vi.fn().mockResolvedValue({ data: { id: 'sub-1', status: 'ACTIVE' }, error: null })
  const limitSub = vi.fn().mockResolvedValue({ data: [{ id: 'sub-1', status: 'ACTIVE' }], error: null })
  const inFn = vi.fn().mockReturnValue({ maybeSingle, limit: limitSub })
  const eq = vi.fn().mockReturnValue({ single, limit, order, in: inFn, eq: vi.fn().mockReturnValue({ single, in: inFn }) })
