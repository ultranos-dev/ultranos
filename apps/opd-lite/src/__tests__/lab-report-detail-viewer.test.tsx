...
// Mock @ultranos/ui-kit/icons (Printer + Check used by LabReportDetail)
...
describe('LabReportDetail — ImageViewer integration', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('renders a clickable image trigger for a webp attachment', () => {
    ...
    render(<LabReportDetail report={report} onBack={vi.fn()} />)

    // The <img> should be present (inside the button trigger)
  ...
  it('opens the ImageViewer when a webp attachment is clicked and audits the view', () => {
    ...
    const img = screen.getByAltText('cell')
    // The button is the parent of the img
    ...
    expect(attachmentAuditCall![2]).toBe('report-test-1')
  ...
  it('does NOT open viewer for PDF attachments', () => {
    const report = makeReport({
      presentedForm: [{ contentType: 'application/pdf', data: 'JVBERi0x', title: 'Report PDF' }],
    })
    render(<LabReportDetail report={report} onBack={vi.fn()} />)

    expect(screen.queryByRole('button', { name: /zoom in/i })).not.toBeInTheDocument()
    const embed = screen.getByTitle('Report PDF')
  ...
  it('emits the detail-view PHI_READ audit on mount (not the attachment audit)', () => {
    ...
    expect(mockAuditPhiAccess).toHaveBeenCalledWith(
      ...
      'report-test-1',
      TEST_PATIENT_ID,
      expect.objectContaining({ phiAccess: 'lab_report_detail_viewed' }),
    )
  })
})
