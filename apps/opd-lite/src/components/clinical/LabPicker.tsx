...
import { searchLabs, type LabSearchResult } from '@/lib/lab-search'
...
export function LabPicker({ value, name, onSelect, onClear }: LabPickerProps) {
  ...
  const handleChange = useCallback((raw: string) => {
    ...
    searchTimerRef.current = setTimeout(async () => {
      const found = await searchLabs(raw)
  ...
  // When a lab is already selected, show a clearable chip instead of the combobox.
  if (value) {
    return (
  ...
  return (
    <div className="relative">
      <input
        ...
        aria-controls="lab-results"
        aria-activedescendant={activeIndex >= 0 ? `lab-option-${activeIndex}` : undefined}
            ...
            return (
              ...
              >
                <div className="font-semibold text-foreground">
                  {highlightMatches(lab.name, getMatchIndices(result.matches, 'name'))}
                </div>
                {lab.accreditationRef && (
                  <div className="text-xs text-muted-foreground">{lab.accreditationRef}</div>
                )}
              </li>
            )
          })}
