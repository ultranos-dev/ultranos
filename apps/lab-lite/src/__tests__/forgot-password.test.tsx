...
describe('ForgotPasswordPage (lab-lite)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('pre-fills email from ?email= query param', () => {
    mockSearchParamsValue = 'email=labtech%40hospital.example'
    render(<ForgotPasswordPage />)
    expect(screen.getByRole('textbox', { name: /email/i })).toHaveValue('labtech@hospital.example')
  })
  ...
  it('calls resetPasswordForEmail with correct redirectTo on submit', async () => {
    ...
    await userEvent.type(screen.getByRole('textbox', { name: /email/i }), 'labtech@hospital.example')
    fireEvent.submit(screen.getByRole('form'))

    await waitFor(() => {
      expect(mockResetPasswordForEmail).toHaveBeenCalledWith('labtech@hospital.example', {
  ...
  it('"Resend" link hidden initially, visible after 60s', async () => {
    ...
    await act(async () => {
      fireEvent.change(screen.getByRole('textbox', { name: /email/i }), {
        target: { value: 'labtech@hospital.example' },
      })
      fireEvent.submit(screen.getByRole('form'))
