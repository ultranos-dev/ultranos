  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      <div className="w-full max-w-lg rounded-3xl bg-white p-6 mx-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-lg font-semibold text-text-primary">Activate Outbreak Mode</h2>
        ...
        {step === 'form' && (
          <>
            {/* Pathogen */}
            <div className="mt-4">
              <label htmlFor="outbreak-pathogen" className="block text-sm font-medium text-text-primary">
                Target Pathogen <span className="text-danger">*</span>
              </label>
              <input
            ...
            <div className="mt-4">
              <span className="block text-sm font-medium text-text-primary">
                Affected Labs <span className="text-danger">*</span>
              </span>
              <div className="mt-2 max-h-48 overflow-y-auto space-y-2">
                {labs.map((lab) => (
                  ...
                  >
                    ...
                    />
                    <span className="text-sm font-medium text-text-primary">{lab.labName}</span>
                    <span className={`ms-auto inline-block rounded-full px-2 py-0.5 text-xs font-medium ${
                      lab.status === 'ACTIVE' ? 'bg-success-subtle text-success'
                        : lab.status === 'PENDING' ? 'bg-warning-subtle text-warning'
            ...
            {/* Notes */}
            <div className="mt-4">
              <label htmlFor="outbreak-notes" className="block text-sm font-medium text-text-primary">
                Notes (optional)
              </label>
              <textarea
                id="outbreak-notes"
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button
                ...
                className="rounded-full bg-danger px-6 py-2.5 text-sm font-semibold text-white disabled:opacity-50 hover:scale-[1.02] transition-transform duration-200"
              >
                Review
              </button>
            </div>
        ...
        {step === 'confirm' && (
          <>
            <div className="mt-4 rounded-xl bg-danger-subtle border border-danger/20 p-4 text-sm text-danger">
