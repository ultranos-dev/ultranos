    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-black">Add Module</h2>
          <button onClick={onClose} className="text-text-muted hover:text-black text-xl leading-none">&times;</button>
        </div>
        ...
        {loading ? (
          <p className="mt-4 text-text-muted">Loading available modules...</p>
        ) : modules.length === 0 ? (
          <div className="mt-4 rounded-3xl border border-dashed border-border bg-surface p-6 text-center">
            <p className="text-text-muted">You&apos;re subscribed to all available modules.</p>
          </div>
        ) : (
          <div className="mt-4 space-y-3">
            {modules.map((mod) => (
              <div key={mod.id} className="rounded-3xl border border-border p-4 hover:border-brand-lime hover:bg-brand-lime/5 transition-colors">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-medium text-black">{mod.displayName}</h3>
                    {mod.description && (
